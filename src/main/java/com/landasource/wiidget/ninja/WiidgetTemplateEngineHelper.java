...
public class WiidgetTemplateEngineHelper {
	...
	public InputStream getResource(final Route route, final Result result) {
		...
		if (result.getTemplate() == null) {
			...
			final String resourcePath = String.format("/%s/%s/%s", parentControllerPackageAsPath, controller.getSimpleName(), viewName);
