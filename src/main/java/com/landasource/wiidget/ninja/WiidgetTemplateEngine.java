...
public class WiidgetTemplateEngine implements TemplateEngine {
    ...
    public void invoke(final Context context, final Result result) {
        ...
        String processedTemplate = context.getRequestPath();
        ...
        try {
            ...
            try {
                ...
                try {
                    processedTemplate = templateEngineHelper.getResource(context.getRoute(), result);

                    if (null == processedTemplate) {

                        final InputStream stream = new FileInputStream(templateEngineHelper.getFile(result.getTemplate()));

                        final String content = IOUtils.toString(stream);
                        try {
                            viewResult = renderer.render(content);
                        } catch (final Exception exception) {
                            viewResult = content;
                        }
                    } else {

                        final InputStream stream = new FileInputStream(processedTemplate);
            ...
            throw new WiidgetException(String.format("Error while process template: %s", processedTemplate), e);
